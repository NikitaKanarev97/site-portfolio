...
Кейс: `/work/learn/` и `/ru/work/learn/`. Основа — реальный редизайн платформы во время работы автора в DSSL / TRASSIR; показана нынешняя переработка для портфолио, а не выпущенная компанией версия. Контекст: `../learn/PROJECT-CONTEXT.md`. Архивный русский каталог и новые экраны различаются как «Исходная платформа / Мой подход сегодня». Локальная приёмка этапа 09 завершена 2026-09-08: 22 WebP, сборка после обеих съёмок, EN/RU кейс и демо проверены. Отчёт и доказательства — `tasks/learn-case/reports/09.md`, автоматическая цепочка закончена; публикация не выполнялась. Уточнение 2026-09-09: пользователь снял замечание к тренировке — сравнивались разные состояния. Приёмка восстановлена, изменения не нужны; история — `tasks/learn-case/reports/09-practice-recheck.md`.

Интерактивный сервис живёт в `/prototypes/learn/home?lang=en`, лендинг — в `/prototypes/learn-landing/?lang=en`. Русский кейс передаёт `lang=ru` в обе ссылки. Эти демо входят в статическую сборку портфолио: отдельный сервер Learn для просмотра не нужен. Для прямых ссылок и перезагрузки экспортируются HTML-входы всех экранов, материалов и шагов обучения.

Исходники сервиса и лендинга — соседний репозиторий `../learn` (лендинг в `landing/app`). После их изменения запустить `npm run sync:learn` или `npm run sync:learn -- D:/path/to/learn`. Команда пересобирает обе локали и обновляет только `public/prototypes/learn`, `learn-landing` и манифест маршрутов. Полученные статические файлы хранятся в репозитории портфолио, поэтому обычный `npm run build` работает без соседнего проекта.

Съёмка: `npm run frames:learn`; проверка переходов, локалей и адаптива: `npm run verify:learn`. Обе команды используют preview на `http://127.0.0.1:4322` (переопределяется `PORTFOLIO_ORIGIN`) и Playwright из `../b2b-dssl` (переопределяется `PLAYWRIGHT_REPO`). Съёмка использует существующие демосостояния из `learn/audit/product-polish/evidence/07`; эти состояния не добавляются в публичный экспорт.
