 ...
 * на канонических 375 px. Механика композита у обоих случаев общая, поэтому колонка
...
/**
 * Ширины берутся не из головы: 1920 — артборд консоли (`shoot-agent-ops-frames.mjs`
 * §VIEWPORT, поднят с 1640 доводкой 09.09.2026), 390 — нижняя граница
 * канонической QA-шкалы адаптива после 360.
 *
...
const CASES = {
  'agent-ops': {
     ...
     * композит читался как незаконченный, а не как узкий экран. С 09.09.2026
     * высота 1200 вслед за артбордом: на 1025 очередь получила внутреннюю
     * прокрутку и теряла футер таблицы.
     */
    height: 1200,
    widths: [1920, 390],
    /**
     * Роль ставится меню роли на индексе, маршрут — pushState внутри SPA.
     * До доводки 09.09.2026 роль выбиралась ссылкой; `PP-006` свёл три ссылки
     * в переключатель, и триггер берётся по `data-track` — `aria-label` кнопки
     * на русской локали переведён, атрибут трекинга нет.
     */
   ...
   * `/ru` в самом прототипе (`shoot-dssl-frames.mjs`). Колонки две:
  ...
  dssl: {
    ...
    /* 1100 с 11.09.2026 — высота артборда кейса (`shoot-dssl-frames.mjs`).
       Было 1300: на прежнем продукте телефонная колонка на 900 обрывалась
       на фильтрах. После доводки (`D032`) шапка телефона — одна строка, и
       первая карточка встаёт выше; десктопный разбор на 1440 кончается на
       1028 (ru 1046) вместе с нижней панелью. На 1300 под ним стояло 300 px
       пустой подложки (проверено кадром), на 1100 первая телефонная карточка
       с подписанными полями входит целиком. */
    height: 1100,
    widths: [1440, 390],
  ...
  /**
   * Единственный мобильный кейс, и диапазон здесь не по ширинам: 375 px —
  ...
  pawly: {
    ...
    columns: [
      ...
      { width: 375, route: '/app/booking-review', height: 980 },
      { width: 375, route: '/app/active-service' },
      { width: 375, route: '/app/order-details' },
    ],
  },
};
