 ...
 *
 * Существует ради одного потребителя: рендера OG-карточек
 * (src/pages/og/[card].png.ts). Satori не исполняет CSS и не знает
 * `var(--text-default)` — ему нужны числа и hex. Прибить их в рендере
 * нельзя: ds/CONTRACT.md §«Что запрещено» запрещает прибитые цвета и
 * кегли на любой поверхности продукта, а карточка соцсетей — поверхность.
 *
